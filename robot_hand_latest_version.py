...
class PPOAgent:
    ...
    def update(self, writer, episode):
        ...
        writer.add_scalar(f"total_loss", total_loss, episode)
...
def plot_training_curves(log_dir, timestamp, num_env):
    ...
    try:
        ...
        plot_configs = [
            ...
            ('total_loss', 'total Loss', gs[1, 0], 'Loss'),
        ]
        for env in range(num_env):
            plot_configs.append((f"Episode_reward_env={env}", f'Episode Rewards of env={env}', gs[2+env, 0], 'Reward'))
            plot_configs.append((f"Average_reward_env={env}", f'Average Rewards of env={env}', gs[2+env, 1], 'Average Reward'))
        ...
        for env in range(num_env):
            main_metrics.append(f"Episode_reward_env={env}")
        aligned_data = {}
...
def evaluate_policy(env, agent, num_episodes=5, num_steps=200):
    """
    ...
    for _ in range(num_episodes):
        ...
        while True:
            ...
            policy_obs = policy_input_converter(obs)
            value_obs = value_input_converter(obs)
            action, _ = agent.get_action(policy_obs, value_obs)
...
def train():
    ...
    env_eval = gym.make('HandManipulateBlockDense-v1', max_episode_steps=EVAL_STEP)
    base_dir = os.getcwd()
    ...
    for episode in range(NUM_EPISODES):
        ...
        for env in range(NUM_ENV):
            writer.add_scalar(f"Episode_reward_env={env}", episode_reward[env]/NUM_STEP, episode)
            writer.add_scalar(f"Average_reward_env={env}", avg_reward[env], episode)
            if episode_reward[env]/NUM_STEP > best_average_reward[env]:
                ...
                print(f"Best reward for {env} saved: {best_average_reward[env]}!")

        if episode % EVAL_INTERVAL == 0:
                eval_metrics = evaluate_policy(env_eval, agent, num_episodes=NUM_EPISODES, num_steps=EVAL_STEP)
        ...
        print('episode', episode, 'avg reward of each env %.1f' % episode_reward/NUM_STEP, 'running_steps', NUM_STEP)
